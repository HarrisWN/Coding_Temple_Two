...
View(mutate(mtcars, new_column = disp*cyl))

View(summarize(mtcars, mtcarsmpg = mean(mpg, na.rm=TRUE)))
...
own_column = flights %>%
  filter(carrier == 'UA') %>%
  group_by (sched_arr_time, carrier) %>%
  summarize(mean = mean(sched_arr_time, na.rm=TRUE))
...
```{r}
ozone_by_month <- airquality %>%
  group_by(Month) %>%
  summarize(airquality, monthly_ozone = mean(Ozone, na.rm=TRUE))
...
mutate(airquality, Ozone_wind_column = Ozone/Wind)
...
```{r}
mutate(airquality, Temp_ozone_column = Temp/Ozone)

```

```{r}

#Hypothesis 1- Ozone levels are highest in the summer
#Hypothesis 2- Higher ozone levels correlate with highter temperatures
#Hypothesis 3 - Wind speed is not correlated with ozone measurements
```
