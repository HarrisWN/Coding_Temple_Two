...
```{r}
# I can type a comment with a single pound sign to be longer than the length of a single line of code and maintain a multi-line comment.
...
```{r}
# The operand for floor division is: %/%
5 %/% 2
...
sub('much', 'little', string, invert=TRUE)
```
...
str2 <- "Joshua scored a 21/25 on his exam as well."

sentence <- paste(str1, str2)
sentence

sprintf("%s scored a %d percent on his exam.", 'Logan', round((22/25)*100, 2))
sprintf("%s scored a %d percent on his exam.", 'Joshua', round((21/25)*100, 2))
```
...
```{r}
names <- c("John", 'Harrison', 'Vaidic', 'Logan', 'Joshua')
names
```
...
```{r}
# Index of 1 into names, we would return John
names[1]
```
...
```{r}
# Generate a sequence of integers from 5 to 15:
sq <- 5:15
sq

# Generate a sequence that contains precision values instead:
my_prec_seq <- 5.5:15.5
my_prec_seq
```
...
```{r}
# Seq from 5 to 15, increase it by increments of .3
...
```{r}
vect_1 <- c(1,5,7,9,10,15,22)
vect_2 <- c(17,25,4,3,9,7,25)
```
...
student_names <- c('Sam', 'Harrison', 'Joshua', 'Logan', 'Vaidic', 'Aliyah')
...
student_overall_grade <- c(65,82,91)

# Create a dataframe from the three vectors:
df <- data.frame(student_name, student_grade_level, student_overall_grade)
df
```
...
```{r}
student_grades <- df$student_overall_grade
student_grades
...
```{r}
# Getting the first and third row as well as the first and second column
df[c(1,3),1:2]
```
...
```{r}
student_name <- c('Jan', 'June')
student_grade_level <- c('Freshman', 'Senior')
student_overall_grade <- c(82,64)
...
my_array <- array(
  ...
  # rows by columns by matricies
  dim=c(3,3,3),
  dimnames=list(c('Row 1', 'Row 2', 'Row 3'), c('Col 1', 'Col 2', 'Col 3'), c('M1', 'M2', 'M3'))
)
my_array
```
...
```{r}
# Get the element in the second row, third column of the first matrix:
my_array[2,3,1]
```

```{r}
# Get the second row of all matrix objects:
my_array[2,,]
```

```{r}
# Get the third column of all matrix objects:
my_array[,3,]
```

```{r}
# Grab the very first matrix:
my_array[c(1,3),,]
```
...
1.  Create an array of two 3x3 matrices from the given vectors.
    ...
    matrix1 <- matrix(vec1, nrow = 3, ncol = 3)
    matrix2 <- matrix(vec2, nrow = 3, ncol =3)

    array_3x3 <- array(c(matrix1, matrix2), dim = c(3,3,2))
...
2.  In the above matrix that you created, print the value 5 by indexing in to the matrix.
    ...
    value1 <- array_3x3[1,2,2]

    value2 <- array_3x3[2,2,2]
...
3.  Print the third row of the first matrix.
    ...
    thirdrowmatrix1 <- array_3x3[1,3, ]
    print(thirdrowmatrix1)
    ```
...
4.  Print the second column of both matrices.
    ...
    second_column_matrix1 <- array_3x3[1, ,2]

    second_column_matrix2 <- array_3x3[2, ,2]

    print(second_column_matrix1)
    print(second_column_matrix2)
